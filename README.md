# 🚀 Streamlit for Machine Learning
A repository of interactive Machine Learning models built with Streamlit.
## 📂 Models
* **SIMPLE-LINEAR-REGRESSION**: Standard OLS approach.
* **MULTIPLE-LINEAR-REGRESSION**: Regression with multiple input features.
* **GRADIENT-DESCENT**: Manual implementation of weight/bias updates.

## ✨ Features
* **Interactive Tuning**: Adjust Learning Rate and Epochs via UI.
* **Live Plots**: Real-time Loss Curves and Best-Fit lines.
* **Accurate Scaling**: Built-in Z-score normalization and de-normalization.

## 🛠️ Quick Start

1. **Install Dependencies**:
   ```bash
   pip install streamlit pandas matplotlib numpy scikit-learn

2. **Run an App**:
   ```bash
   cd GRADIENT-DESCENT
   streamlit run app.py

<div align="center">

## **Happy Coding 🩵**

</div>
